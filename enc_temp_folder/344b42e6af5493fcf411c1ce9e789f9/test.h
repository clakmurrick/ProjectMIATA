	...
	while (isRunning) {
		...
		if (ChangeCar) {
			...
			if (SELECTION == STOCK) {
				...
				drawSlammedMiata((maxX - miata_Width) / 2, (maxY - miata_Height) / 3);
			}
			ChangeCar = false;
			ColorCar = false;
		}
		...
		if (ChangePart) {
			...
			switch (Part) {
			case TopTone:
				drawtopTone((maxX - topTone_Width) / 2, ((maxY / 3) + (maxY / 3)) - topTone_Height);
				break;
			case BottomTone:
				drawbottomTone((maxX - bottomTone_Width) / 2, ((maxY / 3) + (maxY / 3)) - bottomTone_Height);
				break;
			case Interior:
				drawinterior((maxX - interior_Width) / 2, ((maxY / 3) + (maxY / 3)) - interior_Height);
				break;
			case Outline:
				drawoutline((maxX - outline_Width) / 2, ((maxY / 3) + (maxY / 3)) - outline_Height);
				break;
			case Wheels:
				drawwheels((maxX - wheels_Width) / 2, ((maxY / 3) + (maxY / 3)) - wheels_Height);
				break;
			case Windows:
				drawwindows((maxX - windows_Width) / 2, ((maxY / 3) + (maxY / 3)) - windows_Height);
				break;
			case TireWall:
				drawtireWall((maxX - tireWall_Width) / 2, ((maxY / 3) + (maxY / 3)) - tireWall_Height);
				break;
			case TailLights:
				drawtailLights((maxX - tailLights_Width) / 2, ((maxY / 3) + (maxY / 3)) - tailLights_Height);
				break;
			}
			ChangePart = false;
		}
